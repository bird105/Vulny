...
def display(path, payload, vulnerability, line, declaration_text, declaration_line, colored, occurrence, plain):
    ...
    header = "{}潜在的漏洞 : {}{}{}".format('' if plain else '\033[1m', '' if plain else '\033[91m', payload[1], '' if plain else '\033[0m')

    # Line  25  in test/sqli.php
    line = "{}{}{} in {}".format('' if plain else '\033[91m', line, '' if plain else '\033[0m', path)

    # Code : include($_GET['patisserie'])
    vuln = nth_replace("".join(vulnerability), colored, "{}".format('' if plain else '\033[91m') + colored + "{}".format('' if plain else '\033[0m'), occurrence)
    vuln = "{}({})".format(payload[0], vuln)

    # Final Display
    rows, columns = os.popen('stty size', 'r').read().split()
    print("*" * (int(columns) - 1))
    print("名称\t\t{}".format(header))
    # print("-" * (int(columns) - 1))
    ...
    if "$_" not in colored:
        ...
        if declaration_text != "":
            declared = "Line {}{}{} : {}".format('' if plain else '\033[0;91m', declaration_line, '' if plain else '\033[0m', declaration_text)
